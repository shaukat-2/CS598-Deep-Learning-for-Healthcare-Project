...
def get_random_weight(
    ...
    return np.random.uniform(left, right, (dim1,
                             dim2)).astype(config.floatX)


def load_embedding(infile):
    Wemb = np.array(pickle.load(open(infile, 'rb'
...
def init_params(options):
    ...
    if len(timeFile) > 0:
        gruInputDimSize = embDimSize + 1

    params['W_gru_a'] = get_random_weight(gruInputDimSize, 3
            * alphaHiddenDimSize)
    params['U_gru_a'] = get_random_weight(alphaHiddenDimSize, 3
            * alphaHiddenDimSize)
    params['b_gru_a'] = np.zeros(3
                                 * alphaHiddenDimSize).astype(config.floatX)

    params['W_gru_b'] = get_random_weight(gruInputDimSize, 3
            * betaHiddenDimSize)
    params['U_gru_b'] = get_random_weight(betaHiddenDimSize, 3
            * betaHiddenDimSize)
    params['b_gru_b'] = np.zeros(3
                                 * betaHiddenDimSize).astype(config.floatX)
...
def init_tparams(params, options):
    ...
    for (key, value) in params.items():
        if not options['embFineTune'] and key == 'W_emb':
            continue
...
def dropout_layer(
    ...
    proj = T.switch(use_noise, state_before
                    * trng.binomial(state_before.shape, p=keep_prob,
                    n=1, dtype=state_before.dtype) / keep_prob,
                    state_before)
    return proj
...
def gru_layer(
    ...
    def stepFn(wx, h, U_gru):
        uh = T.dot(h, U_gru)
        r = T.nnet.sigmoid(_slice(wx, 0, hiddenDimSize) + _slice(uh, 0,
                           hiddenDimSize))
        z = T.nnet.sigmoid(_slice(wx, 1, hiddenDimSize) + _slice(uh, 1,
                           hiddenDimSize))
        h_tilde = T.tanh(_slice(wx, 2, hiddenDimSize) + r * _slice(uh,
                         2, hiddenDimSize))
    ...
    (results, updates) = theano.scan(
        ...
        outputs_info=T.alloc(numpy_floatX(0.), n_samples,
                             hiddenDimSize),
        non_sequences=[tparams['U_gru_' + name]],
        name='gru_layer',
        n_steps=timesteps,
        )
...
def build_model(tparams, options, W_emb=None):
    ...
    if keep_prob_emb < 1.:
        emb = dropout_layer(emb, use_noise, trng, keep_prob_emb)

    if useTime:  # Adding the time element to the embedding
        temb = T.concatenate([emb, t.reshape([n_timesteps, n_samples,
                             1])], axis=2)
    else:
        temb = emb
    ...
    def attentionStep(att_timesteps):
        ...
        reverse_h_a = gru_layer(tparams, reverse_emb_t, 'a',
                                alphaHiddenDimSize)[::-1] * 0.5
        reverse_h_b = gru_layer(tparams, reverse_emb_t, 'b',
                                betaHiddenDimSize)[::-1] * 0.5

        preAlpha = T.dot(reverse_h_a, tparams['w_alpha']) \
            + tparams['b_alpha']
        preAlpha = preAlpha.reshape((preAlpha.shape[0],
                                    preAlpha.shape[1]))
        alpha = T.nnet.softmax(preAlpha.T).T

        beta = T.tanh(T.dot(reverse_h_b, tparams['W_beta'])
                      + tparams['b_beta'])
        c_t = (alpha[:, :, None] * beta
               * emb[:att_timesteps]).sum(axis=0)
        return c_t

    counts = T.arange(n_timesteps) + 1
    (c_t, updates) = theano.scan(fn=attentionStep, sequences=[counts],
                                 outputs_info=None,
                                 name='attention_layer',
                                 n_steps=n_timesteps)
    if keep_prob_context < 1.:
        c_t = dropout_layer(c_t, use_noise, trng, keep_prob_context)

    preY = T.nnet.sigmoid(T.dot(c_t, tparams['w_output'])
                          + tparams['b_output'])
    ...
    cross_entropy = -(y * T.log(y_hat + logEps) + (1. - y) * T.log(1.
                      - y_hat + logEps))
    cost_noreg = T.mean(cross_entropy)

    cost = cost_noreg + options['L2_output'] * (tparams['w_output']
            ** 2).sum() + options['L2_alpha'] * (tparams['w_alpha']
            ** 2).sum() + options['L2_beta'] * (tparams['W_beta']
            ** 2).sum()

    if options['embFineTune']:
        cost += options['L2_emb'] * (tparams['W_emb'] ** 2).sum()
...
def adadelta(
    ...
    zipped_grads = [theano.shared(p.get_value() * numpy_floatX(0.),
                    name='%s_grad' % k) for (k, p) in
                    tparams.items()]
    running_up2 = [theano.shared(p.get_value() * numpy_floatX(0.),
                   name='%s_rup2' % k) for (k, p) in
                   tparams.items()]
    running_grads2 = [theano.shared(p.get_value() * numpy_floatX(0.),
                      name='%s_rgrad2' % k) for (k, p) in
                      tparams.items()]

    zgup = [(zg, g) for (zg, g) in zip(zipped_grads, grads)]
    rg2up = [(rg2, 0.95 * rg2 + 0.05 * g ** 2) for (rg2, g) in
             zip(running_grads2, grads)]

    if len(options['timeFile']) > 0:
        f_grad_shared = theano.function([x, y, t, lengths], cost,
                updates=zgup + rg2up, name='adadelta_f_grad_shared')
    else:
        f_grad_shared = theano.function([x, y, lengths], cost,
                updates=zgup + rg2up, name='adadelta_f_grad_shared')

    updir = [-T.sqrt(ru2 + 1e-6) / T.sqrt(rg2 + 1e-6) * zg for (zg,
             ru2, rg2) in zip(zipped_grads, running_up2,
             running_grads2)]
    ru2up = [(ru2, 0.95 * ru2 + 0.05 * ud ** 2) for (ru2, ud) in
             zip(running_up2, updir)]
...
def padMatrixWithTime(seqs, times, options):
    ...
    x = np.zeros((maxlen, n_samples, options['inputDimSize'
                 ])).astype(config.floatX)
...
def load_data_simple(seqFile, labelFile, timeFile=''):
    sequences = np.array(pickle.load(open("C:\\Users\\SameenShaukat\\Downloads\\Project\\DL4H-Project\\local_mimic\\save\\pot3digitICD9.seqs", 'rb')))
    labels = np.array(pickle.load(open("C:\\Users\\SameenShaukat\\Downloads\\Project\\DL4H-Project\\local_mimic\\save\\PotAbnFlgs", 'rb')))
    if len(timeFile) > 0:
...
def calculate_auc(test_model, dataset, options):
    ...
    for index in range(n_batches):
        ...
        if useTime:
            batchT = (dataset[2])[index * batchSize:(index + 1)
                * batchSize]
    ...
    ap = average_precision_score(list(labels), list(scoreVec))
    return auc,ap
    ...
    for index in range(n_batches):
        ...
        if useTime:
            batchT = (dataset[2])[index * batchSize:(index + 1)
                * batchSize]
            (x, t, lengths) = padMatrixWithTime(batchX, batchT, options)
            y = np.array((dataset[1])[index * batchSize:(index + 1)
                         * batchSize]).astype(config.floatX)
...
def train_RETAIN(
    ...
    if useTime and embFineTune:
        ...
        if solver == 'adadelta':
            ...
            update_model = theano.function(inputs=[x, y, t, lengths],
                    outputs=cost, updates=updates, name='update_model')
        get_prediction = theano.function(inputs=[x, t, lengths],
                outputs=y_hat, name='get_prediction')
        get_cost = theano.function(inputs=[x, y, t, lengths],
                                   outputs=cost_noreg, name='get_cost')
        ...
        (
            ...
            cost,
            y_hat,
            ) = build_model(tparams, options, W_emb)
        if solver == 'adadelta':
            grads = T.grad(cost, wrt=tparams.values())
            (f_grad_shared, f_update) = adadelta(
                tparams,
        ...
        if solver == 'adadelta':
            ...
            updates = adam(cost, tparams)
            update_model = theano.function(inputs=[x, y, lengths],
                    outputs=cost, updates=updates, name='update_model')
        get_prediction = theano.function(inputs=[x, lengths],
                outputs=y_hat, name='get_prediction')
        get_cost = theano.function(inputs=[x, y, lengths],
                                   outputs=cost_noreg, name='get_cost')
    ...
    print('Loading data ... ')
    if simpleLoad:
        (trainSet, validSet, testSet) = load_data_simple(seqFile,
                labelFile, timeFile)
    else:
    ...
    for epoch in range(max_epochs):
        ...
        for index in random.sample(range(n_batches), n_batches):
            ...
            batchX = (trainSet[0])[index * batchSize:(index + 1)
                * batchSize]
            y = np.array((trainSet[1])[index * batchSize:(index + 1)
                         * batchSize]).astype(config.floatX)

            if useTime:
                batchT = (trainSet[2])[index * batchSize:(index + 1)
                    * batchSize]
                (x, t, lengths) = padMatrixWithTime(batchX, batchT,
                        options)
                if solver == 'adadelta':
        ...
        validAuc, validAP = calculate_auc(get_prediction, validSet, options)
        buf = 'Epoch:%d, Train_cost:%f, Validation_AUC:%f, Validation AP:%f' % (epoch,
                trainCost, validAuc, validAP)
        print (buf)
        print2file(buf, logFile)
        if validAuc > bestValidAuc:
            bestValidAuc = validAuc
            bestValidEpoch = epoch
            bestTestAuc, bestAP = calculate_auc(get_prediction, testSet,
                    options)
            buf = 'Currently the best validation AUC found. Test AUC:%f, Test AP:%f at epoch:%d'% (bestTestAuc,bestAP, epoch)
            print( buf)
            print2file(buf, logFile)
            tempParams = unzip(tparams)
            np.savez_compressed(outFile + '.' + str(epoch),
                                **tempParams)
...
def parse_arguments(parser):
    ...
    parser.add_argument('n_input_codes', type=int,
                        metavar='<n_input_codes>',
    ...
    parser.add_argument('--use_log_time', type=int, default=1,
                        choices=[0, 1],
    ...
    parser.add_argument('--embed_finetune', type=int, default=1,
                        choices=[0, 1],
    ...
    parser.add_argument('--n_epochs', type=int, default=10,
                        help='The number of training epochs (default value: 10)'
                        )
    ...
    parser.add_argument('--solver', type=str, default='adadelta',
...
if __name__ == '__main__':
    ...
    train_RETAIN(  # numClass=args.n_output_codes,
        seqFile=args.seq_file,
        inputDimSize=args.n_input_codes,
        labelFile=args.label_file,
        outFile="C:\\Users\\SameenShaukat\\Downloads\\Project\\DL4H-Project\\local_mimic\\save\\checkpoint\\\PotassiumLabtest\\Retain\\retain",
